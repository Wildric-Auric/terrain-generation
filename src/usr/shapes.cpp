...
void Quad::init() { 
    ...
    std::cout << sizeof(BaseVertexData) << std::endl;
    _indexCount = sizeof(indices) / sizeof(indices[0]);
    _vobj.create(GlobalData::app->data, *GlobalData::cmdBuffPool, (float*)strides.data(), strides.size() * sizeof(strides[0]), 0); 
    ...
    for (ui32 j = 0; j < c - 1; ++j) {
        for (ui32 i = 0; i < c - 1 ; ++i)  {
            ...
            if (i == 0) {
                ...
                *(fvec2*)(&tmp[3]) = fvec2(i * 1.0/c, (j+1) * 1.0/c); 
                _data.addVert(tmp); //up
            }

            *(fvec3*)(&tmp[0]) = l + fvec3(step.x, step.y, 0);
            *(fvec2*)(&tmp[3]) = fvec2((i+1) * 1.0/c, (j+1) * 1.0/c); 
